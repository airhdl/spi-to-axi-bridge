...
| 10    | `don't care` | `status` | `[2] timeout`<br />`[1:0] BRESP` (AXI4 write reponse, only valid when `timeout = 0` |
...
| 6    | `don't care` | `status` | `[2] timeout`<br />`[1:0] RRESP` (AXI4 read response, only valid when `timeout = 0`) |
| 7    | `don't care` | `rd_data[31:24]` | Read data|
| 8    | `don't care` | `rd_data[23:16]` | Read data|
| 9    | `don't care` | `rd_data[15:8]` | Read data|
| 10   | `don't care` | `rd_data[7:0]` | Read data |
